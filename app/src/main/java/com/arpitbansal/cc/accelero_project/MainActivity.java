package com.arpitbansal.cc.accelero_project;
...
public class MainActivity extends AppCompatActivity implements SensorEventListener {
    ...
    public List<ADataType>Data=new ArrayList<>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
       ...
       // graph_y.getViewport().setScalableY(true); // enables vertical zooming and scrolling
        ...
        SGFilter =new SGFilter(polynomialOrder,frameLength);

    }
    public RealMatrix GetFrameDataFromAcceData(int index,int frameSize,List<ADataType>data){
    ...
    public String GetStringValueFromList(List<ADataType>data){
        String s="";
        for (ADataType aDataType :Data){
            s+= aDataType.x+" "+ aDataType.y+" "+ aDataType.z+" "+ aDataType.normal+" "+ aDataType.filtered+"\n";
        }
        return s;
    }
    ...
    class HandlerClass extends Handler {
        ...
        public void handleMessage(Message msg) {
            ADataType aDataType =(ADataType)msg.obj;
            switch (msg.what) {
                case 1:

                    series_y.appendData(
                            new DataPoint(aDataType.index, aDataType.filtered),true,graphSize);

                    return;
                case 2:
                    series_x.appendData(
                            new DataPoint(aDataType.index, aDataType.normal),true,graphSize);
        ...
        if (capturingData && sensorEvent.sensor.getType() == Sensor.TYPE_ACCELEROMETER) {
           /* delay++;
            ...
            if (delay == 0) {
                ...
                ADataType aDataType = new ADataType(index,sensorEvent.values[0], sensorEvent.values[1], sensorEvent.values[2],normal,filteredData);
                ...
                if (normalData.size()>2*frameLength+1) {
                    ...
                    filteredData = SGFilter.GetFiltredData(frameData);
                    aDataType.filtered=filteredData;
                    filteredDataList.add(filteredData);
                    handlerClass.obtainMessage(1, aDataType).sendToTarget();
                    handlerClass.obtainMessage(2, aDataType).sendToTarget();
                    index++;
                }else {
                    handlerClass.obtainMessage(2, aDataType).sendToTarget();
                }
                ...
                Log.d("Sensor", aDataType.toString());
            }
        }
    }
